/*******************************************************************************
Example 06: ADC Input Voltage UDP Sender for M5StickC Plus
...
void loop(){                                    // 繰り返し実行する関数
    ...
    M5.Lcd.setCursor(0, 4, 2);                  // 文字座標と文字サイズ2を設定
    M5.Lcd.println(IP);                         // UDP送信先IPアドレスを表示
    M5.Lcd.setCursor(0, 40, 4);                 // 文字座標と文字サイズ4を設定
    M5.Lcd.drawRect(0, 220, 135, 16, WHITE);    // バー表示枠を描画
    M5.Lcd.fillRect(2, 222, 131 * adc / 4095, 12, GREEN);   // バー表示
    M5.Lcd.printf("adc\n%d\n\n", adc);          // ADC値をLCDに表示
